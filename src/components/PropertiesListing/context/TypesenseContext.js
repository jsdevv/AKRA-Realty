...
export const prices = [
  ...
  { value: [1000000, 5000000], label: '₹ 10L - ₹ 50L', minPrice: 1000000, maxPrice: 5000000 },
  { value: [5000000, 10000000], label: '₹ 50L - ₹ 1Cr', minPrice: 5000000, maxPrice: 10000000 },
  { value: [10000000, 20000000], label: '₹ 1Cr - ₹ 2Cr', minPrice: 10000000, maxPrice: 20000000 },
  { value: [20000000, 40000000], label: '₹ 2Cr - ₹ 4Cr', minPrice: 20000000, maxPrice: 40000000 },
  { value: [40000000, 60000000], label: '₹ 4Cr - ₹ 6Cr', minPrice: 40000000, maxPrice: 60000000 },
  { value: [60000000, 100000000], label: '₹ 6Cr - ₹ 10Cr', minPrice: 60000000, maxPrice: 100000000 },
  { value: [100000000, 150000000], label: '₹ 10Cr - ₹ 15Cr', minPrice: 100000000, maxPrice: 150000000 },
  { value: [150000000, 200000000], label: '₹ 15Cr - ₹ 20Cr', minPrice: 150000000, maxPrice: 200000000 },
  { value: [200000000, 999999999], label: 'Above ₹ 20Cr', minPrice: 200000000, maxPrice: 999999999 }
];
...
export const TypesenseProvider = ({ children }) => {
      ...
      const initialResults = await typesense.typesenseClient
        ...
        .search({
          ...
          filter_by: Object.entries(filters)
            .map(([key, value]) => `${key}:${value}`).filter(f => !f.endsWith('[]')&& !f.trim().endsWith(':'))
            .map(f => `(${f})`)
            .join(" && "),
          query_by:
        ...
        for (let page = 2; page <= totalPages; page++) {
          try {
            ...
            if (pageResult.grouped_hits && pageResult.grouped_hits.length > 0) {
              allResults.grouped_hits = [...allResults.grouped_hits, ...pageResult.grouped_hits];
            } else {
              break;
            }
          } catch (error) {
  ...
  const getGroupedDetailsByIds = async (ids) => {
    ...
    const groupedDetails = await typesense.typesenseClient
      ...
      .search({
        q: "*",
        filter_by: `projectName:=[${ids.join(",")}] && ${appliedFiltersfilters}`,
        query_by:
          "projectName,propertyName,propertyState,propertyCity,propertyZipCode,propertyCardLine2,propertyCardLine3,propertyAddress1,propertyAddress2,locality",
        limit: 250,
    ...
    return groupedDetails.grouped_hits.map((group) => {
      ...
      const stats = group.hits.reduce(
        (acc, hit) => {
          const { amount, bedrooms, sqFt } = hit.document;
      ...
      const formattedProps = {
        ...
        formattedSqFt: formatMinMax(stats.minSqFt, stats.maxSqFt, "Sqft"),
      };
  ...
  const priceRanges = prices.map((range) => `${range.label}:[${range.minPrice},${range.maxPrice}]`).join(",");
    ...
    const facets = await typesense.typesenseClient
      ...
      .search({
        ...
        query_by:
          "projectName",
        limit: 1,
        facet_by: `propertyStatus,propertyType,customStatus,${priceFacet}`,
        include_fields: "id",
...
export const useRefinementList = ({ attribute, sortBy = ["name"], options }) => {
  const { facetData, updateFilters } = useTypesense();
  const [items, setItems] = useState([]);
  ...
  const refine = (value) => {
    ...
    const selectedValues = updatedItems
      ...
      if (isNumericRange) {
        ...
        const filterStringWithAttribute = filterString.replace(`${attribute}:`, '');
